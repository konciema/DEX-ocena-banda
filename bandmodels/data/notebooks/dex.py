...
class Scale:
    ...
    def parse(node):
        ...
        order = Scale.ASC if node.findall(
            "ORDER")[0].text == "ASC" else Scale.DSC
...
class Attribute:
    ...
    def parse(node, scales):
        ...
        parent = (
            node.findall("PARENT")[0].text.strip(
            ) if node.findall("PARENT") else None
        )
        function = (
    ...
    def map_qq(self, val):
        ...
        ind = np.argwhere(
            np.array(list(self.scale.scalevalue.keys())) == val).flatten()
...
class DEXFunction:
    ...
    def __evaluate_QQ(self, **input):
        ...
        exec_ind = (self.__vals == np.array(
            A).round()).all(axis=1).nonzero()[0]

        if len(exec_ind) != 1:
            raise Exception(
                "Wrong number of rules executed %s for rule %s" % (
                    exec_ind, self.name)
            )
...
class DEXModel:
    ...
    def get_input_attributes_detailed(self):
        attributes_detailed = {}
        for value in self.attributes.values():
            if value.level == 1:
                attributes_detailed[value.name] = {
                    'description': value.description,
                    'choices': list(value.scale.scalevalue.keys())
                }
        return attributes_detailed
